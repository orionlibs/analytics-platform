...
```jsonnet
spec.parameters.forProvider.withContents(value)
```
...
###### fn spec.parameters.forProvider.withMatchers

```jsonnet
spec.parameters.forProvider.withMatchers(value)
```
...
###### fn spec.parameters.forProvider.withMatchersMixin

```jsonnet
spec.parameters.forProvider.withMatchersMixin(value)
```
...
```jsonnet
spec.parameters.initProvider.withContents(value)
```
...
(Boolean) Whether the pipeline is enabled for collectors
Whether the pipeline is enabled for collectors
###### fn spec.parameters.initProvider.withMatchers

```jsonnet
spec.parameters.initProvider.withMatchers(value)
```
...
(List of String) Used to match against collectors and assign pipelines to them; follows the syntax of Prometheus Alertmanager matchers
Used to match against collectors and assign pipelines to them; follows the syntax of Prometheus Alertmanager matchers
###### fn spec.parameters.initProvider.withMatchersMixin

```jsonnet
spec.parameters.initProvider.withMatchersMixin(value)
```
