...
import { GrafanaTheme2, PanelData, VisualizationSuggestion, FieldConfigSource } from '@grafana/data';
...
export function VisualizationSuggestionCard({ data, suggestion, onChange, width }: Props) {
  ...
  // always set img src for table to avoid showing an error
  if (suggestion.pluginId === 'table') {
    cardOptions.imgSrc = 'public/app/plugins/panel/table/img/icn-table-panel.svg';
  }
    ...
    <button {...commonButtonProps}>
      <Tooltip content={suggestion.name}>
        <div style={innerStyles} className={styles.renderContainer}>
          <PanelRenderer title="" data={data} pluginId={suggestion.pluginId} width={renderWidth} height={renderHeight} options={preview.options} fieldConfig={preview.fieldConfig} />
