...
(async () => {
    ...
    const databaseAction = async (action) => {
        ...
        try {
            if (action.method === Database.GET) {
                ...
                return await client.query(`INSERT INTO ${action.table}(name)
                                           VALUES ($1)`, [action.name]);
            } else if (action.method === Database.DELETE) {
                return await client.query(`DELETE
                                           FROM ${action.table}
                                           WHERE name = $1`, [action.name]);
    ...
    // Simple health check endpoint (minimal response for basic checks)
    ...
    app.post('/:endpoint', async (req, res) => {
        ...
        try {
            ...
            logEntry({
                ...
                endpointLabel: spanTag,
                endpoint,
                message: `traceID=${traceId} http.method=POST endpoint=${endpoint} status=SUCCESS name="${name}"`,
            });
    ...
    const teardownCheck = (details) => {
        ...
        if (teardownInProgress) {
            logEntry({
                ...
                namespace: process.env.NAMESPACE,
                job: `${servicePrefix}-server`,
                message: `traceID=${spanContext.traceId} http.method=${method} endpoint=${endpoint} status=FAILURE error='Table is not available'`,
            });
    ...
    const createConnectionPool = async (maxRetries = 10, retryDelay = 5000) => {
        let retryCount = 0;
        ...
        while (retryCount < maxRetries) {
            try {
                logEntry({
                    ...
                    job: `${servicePrefix}-server`,
                    namespace: process.env.NAMESPACE,
                    message: `Attempting to create database connection pool (attempt ${retryCount + 1}/${maxRetries})...`,
                });
                ...
                const pool = new Pool({
                    ...
                    database: process.env.MYTHICAL_DATABASE_NAME || 'postgres',
                    min: Number(process.env.DB_POOL_MIN) || 2,
                    max: Number(process.env.DB_POOL_MAX) || 100, // This is quite high, so that we try to make the database fall over
                    connectionTimeoutMillis: Number(process.env.DB_CONNECTION_TIMEOUT) || 30000,
                    idleTimeoutMillis: Number(process.env.DB_IDLE_TIMEOUT) || 10000, // 10 secs
                    query_timeout: Number(process.env.DB_QUERY_TIMEOUT) || 5000, // 5 secs
                    statement_timeout: Number(process.env.DB_STATEMENT_TIMEOUT) || 5000, // 5 secs
                });
                ...
                logEntry({
                    ...
                    job: `${servicePrefix}-server`,
                    namespace: process.env.NAMESPACE,
                    message: `Failed to create database connection pool (attempt ${retryCount}/${maxRetries}): ${error.message}`,
                });

                if (retryCount >= maxRetries) {
                    throw new Error(`Failed to create database connection pool after ${maxRetries} attempts: ${error.message}`);
                }
        ...
        await api.context.with(api.trace.setSpan(api.context.active(), requestSpan), async () => {
            try {
                logEntry({
                    ...
                    job: `${servicePrefix}-server`,
                    namespace: process.env.NAMESPACE,
                    message: 'Creating postgres connection pool...',
                });

                pgPool = await createConnectionPool();
